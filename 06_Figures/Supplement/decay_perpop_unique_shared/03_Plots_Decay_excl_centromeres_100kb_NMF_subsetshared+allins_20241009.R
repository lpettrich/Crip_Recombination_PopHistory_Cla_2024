...
#---------------------------
# Read in inserts
inserts <- read.csv("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/bedtools-closest/run12/MELT_sharedsubsetins.csv", sep = ",", header = TRUE)
inserts2 <- read.csv("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/bedtools-closest/run12/MELT_sharedallins.csv", sep = ",", header = TRUE)

head(inserts)
tail(inserts)

inserts <- rbind(inserts, inserts2)

unique(inserts$pops)

# Filter inserts of dfm
dfm <- inner_join(dfm, inserts, by = c("no_insert"))
...
c1.1_blankx <- c1.1 + scale_y_continuous(n.breaks = 5, limits=c(0, 250)) + theme_blankx + labs(title = "Chromosome 1, arm 1") +
  annotate("text", x = Inf, y = Inf, 
           label = paste("n =", sum(dfm$chromosome_arm == "Chr1_arm1")), 
           hjust = 1.1, vjust = 1.5, size = 5, color = "black")

c2.1_blankx <- c2.1 + scale_y_continuous(n.breaks = 5, limits=c(0, 250)) + theme_blankx + labs(title = "Chromosome 2, arm 1")+
  annotate("text", x = Inf, y = Inf, 
           label = paste("n =", sum(dfm$chromosome_arm == "Chr2_arm1")), 
           hjust = 1.1, vjust = 1.5, size = 5, color = "black")

c3.1_blankx <- c3.1 + scale_y_continuous(n.breaks = 5, limits=c(0, 250)) + theme_blankx + labs(title = "Chromosome 3, arm 1") +
  annotate("text", x = Inf, y = Inf, 
           label = paste("n =", sum(dfm$chromosome_arm == "Chr3_arm1")), 
           hjust = 1.1, vjust = 1.5, size = 5, color = "black")

c1.2_blankx <- c1.2 + scale_y_continuous(n.breaks = 5, limits=c(0, 250)) + theme_blankx + labs(title = "Chromosome 1, arm 2") +
  annotate("text", x = Inf, y = Inf, 
           label = paste("n =", sum(dfm$chromosome_arm == "Chr1_arm2")), 
           hjust = 1.1, vjust = 1.5, size = 5, color = "black")

c2.2_blankx <- c2.2 + scale_y_continuous(n.breaks = 5, limits=c(0, 250)) + theme_blankx + labs(title = "Chromosome 2, arm 2") +
  annotate("text", x = Inf, y = Inf, 
           label = paste("n =", sum(dfm$chromosome_arm == "Chr2_arm2")), 
           hjust = 1.1, vjust = 1.5, size = 5, color = "black")

c3.2_blankx <- c3.2 + scale_y_continuous(n.breaks = 5, limits=c(0, 250)) + theme_blankx + labs(title = "Chromosome 3, arm 2")+
  annotate("text", x = Inf, y = Inf, 
           label = paste("n =", sum(dfm$chromosome_arm == "Chr3_arm2")), 
           hjust = 1.1, vjust = 1.5, size = 5, color = "black")

c4 <- c4 + scale_y_continuous(n.breaks = 5, limits=c(0, 250)) + labs(title = "Chromosome 4") +
  annotate("text", x = Inf, y = Inf, 
           label = paste("n =", sum(dfm$chromosome_arm == "Chr4_NA")), 
           hjust = 1.1, vjust = 1.5, size = 5, color = "black")

p_merged <- c1.1_blankx + 
  c1.2_blankx +
  c2.1_blankx + 
  c2.2_blankx +
  c3.1_blankx +  
  c3.2_blankx +
  c4 +
  patchwork::plot_layout(ncol = 1, nrow = 7, heights = c(1,1))

p_merged

save_plot("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/DecayPlots_run12_arms_100kb_NMF_subset_allsharedins.png", p_merged, nrow = 5, ncol = 1, base_asp = 1.9, dpi = 500, bg = "white")
save_plot("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/DecayPlots_run12_arms_100kb_NMF_subset_allsharedins.svg", p_merged, nrow = 5, ncol = 1, base_asp = 1.9, dpi = 500, bg = "white")



c4<- c4 + scale_y_continuous(n.breaks = 5, limits=c(0, 230)) + labs(title = "Chromosome 4")
save_plot("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/DecayPlots_run12_arms_100kb_NMF_chr4_subset_allsharedins.png", c4, base_asp = 1.9, dpi = 500, bg = "white")
save_plot("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/DecayPlots_run12_arms_100kb_NMF_chr4_subset_allsharedins.svg", c4, base_asp = 1.9, dpi = 500, bg = "white")
