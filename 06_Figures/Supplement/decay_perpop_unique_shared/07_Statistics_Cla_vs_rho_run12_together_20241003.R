...
# Read data
df1 <- read.table("Pearson_Correlation_mean_rho_vs_distance-to-Cla_run12_unique_20241009.txt", sep = ";")
df2 <- read.table("Pearson_Correlation_mean_rho_vs_distance-to-Cla_run12_shared_20241009.txt", sep = ";")
...
save_plot("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/heatmap_run12_unique_shared_20241009.png", p, nrow = 3, ncol = 2, base_asp = 1.2, dpi = 500, bg = "white")

save_plot("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/heatmap_run12_unique_shared_20241009.svg", p, nrow = 3, ncol = 2, base_asp = 1.2, dpi = 500, bg = "white")
...
results_df2$filter_f  <- factor(results_df2$filter, levels = c("unique", "shared"))
...
p <- ggplot() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        ...
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) +
  facet_grid(facet_label ~ filter_f, scales = "fixed", switch = "y")   +  # Facet based on chromosome and arm
  theme(strip.text.y.left = element_text(angle = 0, hjust = 1))  # Angle y-axis facet labels


p

save_plot("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/heatmap_run12_noNAarm_unique_shared_20241009.png", p, nrow = 3, ncol = 2, base_asp = 1.2, dpi = 500, bg = "white")

save_plot("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/heatmap_run12_noNAarm_unique_shared_20241009.svg", p, nrow = 3, ncol = 2, base_asp = 1.2, dpi = 500, bg = "white")
