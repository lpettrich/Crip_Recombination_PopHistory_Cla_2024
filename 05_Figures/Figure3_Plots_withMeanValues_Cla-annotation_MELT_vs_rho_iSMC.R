# The interplay of recombination landscape, 
# transposable elements and population history 
# in European populations of Chironomus riparius
############################################################
#                                                          #
...
#-------------------------------------------------------------------
# Rename columns to individual names
#-------------------------------------------------------------------
...
# Check if it worked, only 10e3 should remain as window size
unique(df$wnd_size)
...
lp <- ggplot(legend_df, aes(y=legend_df$pop, x=V3, color=legend_df$pop)) +
  geom_line(size=0.5) +
...
save_plot("/home/alle/sciebo/RecombinationLandscape_CRIP/Plots/png-files/Figure3_rec-landscape_and_Cla-element_v2.png", pf2, nrow = 8, ncol = 4, base_asp = 1.8, dpi = 500, bg = "white", scale = 0.6)
