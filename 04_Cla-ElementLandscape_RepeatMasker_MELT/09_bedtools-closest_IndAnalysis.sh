...
OUTDIR=/projects/ag-waldvogel/pophistory/CRIP/comparison_Cla_rho/01-bedtools_closest/run07_Indiv

CLADIR=/projects/ag-waldvogel/pophistory/CRIP/MELT-analysis/Cla1/AllPop/1-IndivAnalysis

INPDIR=/projects/ag-waldvogel/pophistory/CRIP/MELT-analysis/Cla1/AllPop/1-IndivAnalysis
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${CLADIR}
    cat "${ind}".bwamem.sambl.sort.Cla1.tmp.bed | grep "${chr}" | sort -k1,1 -k2,2n > "${ind}"_"${chr}".bwamem.sambl.sort.Cla1.tmp.sorted.bed      
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    bedtools closest -d -t first -a ${RHODIR}/bed-files/"${ind}"_"${chr}"_ismc.rho.10kb.bedgraph_renamed.sorted.bed \
    -b ${INPDIR}/"${ind}"_"${chr}".bwamem.sambl.sort.Cla1.tmp.sorted.bed > \
    "${ind}"_"${chr}"_ismc.rho.10kb.bedgraph_renamed.sorted_distToNearestCla1.bed
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${OUTDIR}
    awk '$7 - $6 >= 500' "${ind}"_"${chr}"_ismc.rho.10kb.bedgraph_renamed.sorted_distToNearestCla1.bed > "${ind}"_"${chr}"_ismc.rho.10kb.bedgraph_renamed.sorted_distToNearestCla1_more500bp.bed
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${OUTDIR}
    awk '$7 - $6 < 500' "${ind}"_"${chr}"_ismc.rho.10kb.bedgraph_renamed.sorted_distToNearestCla1.bed > "${ind}"_"${chr}"_ismc.rho.10kb.bedgraph_renamed.sorted_distToNearestCla1_less500bp.bed
  done
done
