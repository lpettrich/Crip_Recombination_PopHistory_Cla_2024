...
OUTDIR=/projects/ag-waldvogel/pophistory/CRIP/comparison_Cla_rho/01-bedtools_closest/run12
...
### Cla regions compared to rho (100 kb)
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    ...
    "${ind}"_"${chr}"_ismc.rho.100kb.bedgraph_renamed.sorted_distToNearestCla1.bed
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${OUTDIR}
    awk '$7 - $6 >= 500' "${ind}"_"${chr}"_ismc.rho.100kb.bedgraph_renamed.sorted_distToNearestCla1.bed > "${ind}"_"${chr}"_ismc.rho.100kb.bedgraph_renamed.sorted_distToNearestCla1_more500bp.bed
  done
done
...
for ind in "${IND[@]}"; do
  for chr in "${CHR[@]}"; do
    cd ${OUTDIR}
    awk '$7 - $6 < 500' "${ind}"_"${chr}"_ismc.rho.100kb.bedgraph_renamed.sorted_distToNearestCla1.bed > "${ind}"_"${chr}"_ismc.rho.100kb.bedgraph_renamed.sorted_distToNearestCla1_less500bp.bed
  done
done
