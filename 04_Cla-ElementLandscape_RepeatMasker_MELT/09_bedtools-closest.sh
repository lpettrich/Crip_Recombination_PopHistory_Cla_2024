...
for pop in "${POP[@]}"; do
  for chr in "${CHR[@]}"; do
    ...
    -b ${CLADIR}/bed-files/"${pop}"_"${chr}"_Cla1.final_comp_noac0_nos25.sorted.bed > "${pop}"_"${chr}"_ismc.rho.10kb.bedgraph_renamed.sorted.merged.mean_distToNearestCla1.bed
  done
done
